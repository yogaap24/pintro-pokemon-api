...
func (p pgConfig) ConnStr() string {
	return fmt.Sprintf("user=%s password=%s host=%s port=%d database=%s sslmode=%s", p.User, p.Password, p.Host, p.Port, p.DBName, p.SslMode)
}
